...
import (
	...
	"github.com/sosalejandro/algo-practice/graph/dfs"
)
...
func TestDeepFirstSearch(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			start:    "A",
			expected: []string{"A", "B", "D", "E", "F", "C"},
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			itemFactory := dfs.StackStringItemFactory
			result := dfs.DeepFirstSearch(tt.graph, tt.start, itemFactory)
